...
describe('Modules', () => {
  describe('module registration', () => {
    it('dynamic module registration', () => {
      ...
      expect(store.$store._mutations.inc.length).toBe(2);
      expect(store.hi.a).toBe(1);
      ...
      store.$store.dispatch('inc');
      expect(store.hi.a).toBe(2);
      ...
      expect(store.hi).toBeUndefined();
      expect(store.a).toBeUndefined();
      expect(store.$store._mutations.inc.length).toBe(1);
      expect(store.$store._actions.inc).toBeUndefined();

      // assert initial modules still work as expected after unregister
      store.$store.dispatch('incFoo');
      expect(store.foo.bar).toBe(3);
      // expect(store.bar).toBe(3);
    });
    ...
    //   expect(store['a/foo']).toBe(1);

    //   store.$store.dispatch('a/foo');
    //   expect(actionSpy).toHaveBeenCalled();

    //   store.$store.commit('a/foo');
    ...
    it('dynamic module existance test', () => {
      ...
      expect(store.$store.hasModule('bonjour')).toBe(true);
      store.unregisterModule('bonjour');
      expect(store.$store.hasModule('bonjour')).toBe(false);
    });
    ...
    //   expect(store.$store.hasModule(['a'])).toBe(true);
    //   expect(store.$store.hasModule(['a', 'b'])).toBe(true);
    //   expect(store.$store.hasModule(['c'])).toBe(false);
    //   expect(store.$store.hasModule(['c', 'd'])).toBe(false);
    // });

    it('dynamic module registration preserving hydration', () => {
      const store = new Vuex.Store({});
      store.$store.replaceState({ a: { foo: 'state' } });
      const actionSpy = jest.fn();
      const mutationSpy = jest.fn();
  ...
  //   store.$store.watch((state) => state.a, spy);
  ...
  describe('modules usage', () => {
    it('state as function (multiple module in same store)', () => {
      ...
      expect(store.one.a).toBe(0);
      expect(store.two.a).toBe(0);

      store.$store.commit(TEST, 1);
      expect(store.one.a).toBe(1);
      expect(store.two.a).toBe(1);
    });
    ...
    it('state as function (same module in multiple stores)', () => {
      ...
      expect(storeA.foo.a).toBe(0);
      expect(storeB.bar.a).toBe(0);

      storeA.$store.commit(TEST, 1);
      expect(storeA.foo.a).toBe(1);
      expect(storeB.bar.a).toBe(0);

      storeB.$store.commit(TEST, 2);
      expect(storeA.foo.a).toBe(1);
      expect(storeB.bar.a).toBe(2);
    });
    ...
    it('module: mutation', function () {
      ...
      store.$store.commit(TEST, 1);
      expect(store.a).toBe(2);
      expect(store.nested.a).toBe(3);
      expect(store.nested.one.a).toBe(4);
      expect(store.nested.nested.two.a).toBe(5);
      expect(store.nested.nested.three.a).toBe(6);
      expect(store.four.a).toBe(7);
    });
    ...
    it('module: action', function () {
      ...
      const makeAction = (n) => {
        return {
          [TEST]({ state, rootState }) {
            ...
            expect(rootState).toBe(store.$store.state);
          },
        };
      };
      const store = new Vuex.Store({
      ...
      store.$store.dispatch(TEST);
      expect(calls).toBe(6);
    });

    // it('module: getters', function () {
    //   const makeGetter = (n, f) => ({
    ...
    //       nested: {
    //         state: { b: 2 },
    //         getters: makeGetter(2, 'b'),
    //         modules: {
    //           one: {
    //             state: { c: 3 },
    //             getters: makeGetter(3, 'c'),
    //           },
    //           nested: {
    //             modules: {
    //               two: {
    //                 state: { d: 4 },
    //                 getters: makeGetter(4, 'd'),
    //               },
    //               three: {
    //                 state: { e: 5 },
    //                 getters: makeGetter(5, 'e'),
    //               },
    //             },
    //           },
    //         },
    ...
    it('module: namespace', () => {
      ...
      expect(store.a.a).toBe(1);
      expect(store.a.b).toBe(2);
      store.$store.dispatch('a/' + TEST);
      expect(actionSpy).toHaveBeenCalled();
      store.$store.commit('a/' + TEST);
      expect(mutationSpy).toHaveBeenCalled();
    });
    ...
    it('module: nested namespace', () => {
      ...
      //   expect(store[type]).toBe(true);
      // });
      expect(store.a.a).toBe(true);
      expect(store.a.b.b).toBe(true);
      expect(store.a.b.c.c).toBe(true);
      expect(store.a.d.d).toBe(true);
    ...
    it('module: getters are namespaced in namespaced module', () => {
      ...
      expect(store.a.foo).toBe('module');
      expect(store.a.bar).toBe('module');
      expect(store.a.baz).toBe('root');
    });
    ...
    it('module: use other module that has same namespace', (done) => {
          ...
          parent: {
            ...
            modules: {
              a: {
                state: { value: 'a' },
                getters: { foo: (state) => state.value },
                actions: { foo1: actionSpy },
                mutations: { foo2: mutationSpy },
              },
      ...
      store.$store.dispatch('parent/test');
    });
    ...
    it('dispatching multiple actions in different modules', (done) => {
      ...
      store.$store.dispatch(TEST).then((res) => {
        expect(res[0]).toBe(1);
        expect(res[1]).toBe(2);
        done();
      });
  ...
  it('action error subscribers', (done) => {
    ...
    store.$store.dispatch(TEST, 2).catch(() => {
      expect(beforeSpy).toHaveBeenCalledWith(
        { type: TEST, payload: 2 },
        store.$store.state
      );
